...
var cacheExchange = function(ref) {
  ...
  return function(ops$) {
    ...
    var forwardedOps$ = wonka.pipe(wonka.merge([ wonka.pipe(sharedOps$, wonka.filter(_ref4), wonka.map(mapTypeNames)), wonka.pipe(sharedOps$, wonka.filter(_ref5)) ]), wonka.map(_ref6), forward, wonka.tap(_ref7));
...
Client.prototype.executeRequestOperation = function executeRequestOperation(operation) {
  ...
  return this.suspense ? (source = result$, wonka.make(function(ref) {
    var end = ref[1];
    var isCancelled = !1;
    var resolveSuspense;
...
var useImmediateState = function(init) {
  ...
  !isSSR && React.useLayoutEffect(function() {
    isMounted.current = !0;
    return _ref;
  }, []);
...
var isInternalComponent = function(Component) {
...
var useMutation = function(query) {
  ...
  return [ ref[0], React.useCallback(function(variables) {
    ...
    return wonka.pipe(client.executeMutation(request, devtoolsContext), wonka.toPromise).then(_ref);
...
var useQuery = function(args) {
  ...
  var executeQuery = React.useCallback(function(opts) {
    ...
    }, opts, devtoolsContext)), wonka.subscribe(_ref2)), unsubscribe.current = assign[0];
...
var useSubscription = function(args, handler) {
  ...
  var executeSubscription = React.useCallback(function() {
    var assign;
    unsubscribe.current();
    assign = wonka.pipe(client.executeSubscription(request, devtoolsContext), wonka.subscribe(_ref)), 
